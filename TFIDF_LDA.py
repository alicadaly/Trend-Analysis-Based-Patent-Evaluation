...
def main(options, args):
	...
	if options.no_below!=0 or options.no_above!=1.0 or options.keep_n != None:
		Dic = DicCompact(Dic,options.no_below,options.no_above,options.keep_n)
	else:
		if not options.dictionary:
			Dic.save(directory+'/dict.dict')
...
def PatentData2Dic(parseddata):
	dictionary = corpora.Dictionary(line.split() for line in open(directory + parseddata))
...
def Patent2Corpus(dictionary, data):
	class MyCorpus(object):
		def __iter__(self):
			for line in open(directory + data):
				# assume there's one document per line, tokens separated by whitespace
				yield dictionary.doc2bow(line[8:].split())
	corpus_memory_fiendly = MyCorpus()
	corpora.MmCorpus.serialize(directory+'/corpus.mm', corpus_memory_fiendly)
...
def saveLDA_Value(ldaValue):
	writefile = open(directory+'/lda_value.csv','w')
	writer = csv.writer(writefile)
...
def saveTopics(lda):
	writefile = open(directory+'/lda_topics.csv','w')
	writer = csv.writer(writefile)
